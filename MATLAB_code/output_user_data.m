...
%Author: Kevin Müller, 05.04.2020
    ...
    for m1 = 1 : size(info_link,1)
        ...
        if info_link(m1,2) > 1
            ...
            user_data.links.('Health state')(m1) = name_health_state(db_cv.node.d.('Health state')(info_link(m1,1))) ;
            user_data.links.('Social distance')(m1) = name_s_distance(cv_distance) ;
        else
            user_data.links.('Health state')(m1) = 'No information' ;
            user_data.links.('Social distance')(m1) = 'No information' ;
        end
    end
